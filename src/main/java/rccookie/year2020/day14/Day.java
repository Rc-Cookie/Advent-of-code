package rccookie.year2020.day14;
...
public class Day extends com.github.rccookie.adventofcode.util.Day {
    ...
    public long resultPart1() throws Exception {
        HashMap<Integer, boolean[]> memory = new HashMap<>(input.length);
        Boolean[] mask = new Boolean[36];
        for(String line : input) {
            if(line.startsWith("mask")) {
                char[] bits = line.substring(7).toCharArray();
                for(int i=0; i<bits.length; i++) {
                    mask[i] = bits[i] == 'X' ? null : bits[i] == '1';
                }
            }
            else {
                boolean[] number = toBinary(Long.parseLong(line.substring(line.indexOf('=') + 2)));
                boolean[] masked = new boolean[36];
                for(int i=0; i<mask.length; i++) masked[i] = mask[i] == null ? number[i] : mask[i];
                memory.put(Integer.parseInt(line.substring(4, line.indexOf(']'))), masked);
            }
        }
        return memory.values().stream().mapToLong(Day::toDecimal).sum();
    }
    ...
    public long resultPart2() throws Exception {
        ...
        for(String line : input) {
            if(line.startsWith("mask")) {
                ...
                int storedValue = Integer.parseInt(line.substring(line.indexOf('=') + 2));
                int address = Integer.parseInt(line.substring(4, line.indexOf(']')));
                boolean[] bitAddress = toBinary(address);
                Boolean[] masked = new Boolean[bitAddress.length];
                int comCount = 1;
                for(int i=0; i<mask.length; i++) {
                    ...
                    else masked[i] = mask[i] || bitAddress[i];
                }
                for(int i=0; i<comCount; i++) {
                    boolean[] combination = new boolean[36];
                    int n = 0;
                    for(int j=0; j<mask.length; j++) {
                        if(masked[j] != null) combination[j] = masked[j];
                        else combination[j] = (i / (int)Math.pow(2, n++)) % 2 == 1;
                    }
                    long comAddress = toDecimal(combination);
                    memory.put(comAddress, storedValue);
                }
            }
        }
    ...
    private static boolean[] toBinary(long number) {
        boolean[] bits = new boolean[36];
        for(int i=0; number != 0; i++) {
            double pow = Math.pow(2, bits.length - i - 1);
            if(number >= pow) {
                number -= pow;
                bits[i] = true;
            }
    ...
    private static long toDecimal(boolean[] bits) {
        long number = 0;
        for(int i=0; i<bits.length; i++) if(bits[i]) number += Math.pow(2, bits.length - i - 1);
        return number;
    }
}
